...
function buildMessages(spec, target) {
  const system = [
    ...
    'Honor the @machine section — when a cmd is a reducer (current state, event) -> next state, implement the dispatch (per-event-key transitions), the state transitions (from->to with triggers/guards), and the derived-field mappings exactly as specified.',
