...
- **첫 실측 — Rimlog 브랜딩·듀얼 컬러셋 사이클 (2026-07-05)**: `rimlog-app.eux`(~40 L)에 라이트/다크 컬러셋·앱 타이틀 서술 추가 → 재brew 로 산출 `rimlog-app.js` 의 CSS 변수 체계(`--rim-*` 듀얼 셋) + 하위 UV 위젯 변수 주입(`--aic-*`) 전반 갱신, `ai-insight-card.eux` 는 @ports 격리 유지한 채 변수 관통만 반영. sha 라운드트립(dist 헤더 sha 교체)·drift in-sync 유지. **관찰**: UI 표면 기준 대수술(테마 체계 도입)이 spec 레벨에선 서술 몇 줄 추가로 수렴 — 유지보수 지점이 spec 으로 모이는 X4 가설의 첫 정량 사례. 단 같은 사이클에서 agent 의 프레임워크 API 오인(토스트 API — 전역 `note()` 가 정답) 2건이 **스모크에서** 검출·정정됨 — spec-수정 사이클에도 산출 검증 게이트(스모크/실행 확인)는 생략 불가.
