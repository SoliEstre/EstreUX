...
| **주력 채널 값** | `:` 뒤의 일반 토큰. 한 배정에 정확히 하나여야 해요. 현재 투영 규칙이 있는 값은 `skill`·`mcp`·`hook` 세 가지예요. 주력 채널이 `skill` 이면 바로 뒤에 실물 이름을 `skill(실물이름)` 형식으로 **반드시** 적어야 해요 — 예를 들어 `brewGuide : skill(estreux-brew)` 는 `skills/estreux-brew/SKILL.md` 를 가리켜요. 생략하면 FAIL 이에요. **부재를 「기능 키와 일치」로 간주하지 않아요** — 이름이 어긋난 배치에서 조용히 엉뚱한 경로를 재게 되고(협업 프로젝트 실측: 설치본 114 스킬 중 디렉터리명 ≠ 정본 이름 3건), 그건 `none` 이 세운 원칙 — 「없음」과 「안 적음」을 한 모양으로 만들지 않는다 — 을 이름 축에서 되살리는 일이에요. |
...
| **`pointer(채널)`** | 주력이 아닌 다른 노출 표면을 가리키는 표기. 주력 채널 개수에는 포함되지 않지만, 가리킨 채널의 실물 존재 검사와 manifest 채널 집합에는 포함돼요. 포인터 파일이나 내용을 생성하지는 않아요. **괄호의 뜻이 위치에 따라 다릅니다.** 주력 채널 뒤 `skill(estreux-brew)` 의 괄호는 **실물 이름**, `pointer(skill)` 의 괄호는 **채널 이름**이에요. 포인터에는 별도 실물 이름을 지정하지 않으므로 `pointer(skill)` 은 기능 키를 Skill 실물 이름으로 사용해요. |
| **`none: <사유>`** | 이 spec 이 어느 노출 표면에도 직접 기여하지 않음을 명시하는 디렉티브 인라인 값. 사유는 주석이 아니라 값이며, 주석 제거 뒤에도 비어 있지 않아야 해요. 배정 블록과 함께 쓸 수 없어요. |

배정 행의 전체 형식은 `기능키 : 채널[(실물이름)][, pointer(채널)...]` 이에요. 채널 목록은 `,` 로 나누고 각 토큰의 앞뒤 공백과 빈 토큰을 버려요. `pointer` 는 `pointer(` 바로 뒤에 채널 식별자를 쓰는 형식이고 괄호 안쪽 공백은 허용해요. `#` 앞에 공백이 있는 행 끝 주석과 주석 전용 행을 제거하므로, `#`로 바로 시작하는 행은 주석으로 인식하지 않아요. `none` 의 사유도 이 주석 제거 뒤에 남은 값으로 판정하므로 주석은 사유로 세지 않아요.
...
- 배정 행은 `기능키 : 채널[(실물이름)][, pointer(채널)...]` 형식이어야 해요. `@channels` 뒤 다음 `@directive` 전까지의 비어 있지 않은 다른 형식 행은 FAIL 이에요.
